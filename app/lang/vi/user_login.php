<?php

return array(

    'page_title' => 'Đăng nhập',
    'customer_id' => 'Mã học viên',
    'password' => 'Mật khẩu',
    'remember_me' => 'Ghi nhớ',
    'remember_me_warning_msg' => 'Cảnh báo: Chức năng này chỉ nên sử dụng trên máy tính cá nhân!',
    'login' => 'Đăng nhập',
    'account_is_not_a_customer_error_msg' => 'Tài khoản này không dành cho hệ thống Portal. Vui lòng đăng nhập vào CRM!',
    'login_failed_error_msg' => 'Đăng nhập không thành công. Vui lòng thử lại!',
    'description' => '<p>Để sử dụng hệ thống, xin vui lòng đăng nhập với <strong>mã học viên</strong> và <strong>mật khẩu</strong> được cung cấp khi đăng ký sử dụng dịch vụ.</p><p>Nếu không thể đăng nhập vào hệ thống hoặc không có tài khoản, xin vui lòng liên hệ bộ phận kỹ thuật để được giúp đỡ.</p><p>Xin chân thành cảm ơn!</p>',
    'southern_sales' => 'Kinh doanh Miền Nam',
    'northern_sales' => 'Kinh doanh Miền Bắc',
    'technical_supports' => 'Hỗ trợ kỹ thuật',
    'customer_services' => 'Dịch vụ khách hàng',
);